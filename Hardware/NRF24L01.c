...
uint8_t TX_ADDRESS[TX_ADR_WIDTH] = {0x68,0x86,0x66,0x88,0x28};  // 定义一个静态发送地址，这个地址是5个字节，40位（因为地址位宽设置成了5），应该是随便哪五字节
uint8_t RX_ADDRESS[RX_ADR_WIDTH] = {0x68,0x86,0x66,0x88,0x28};	// 收发是同一个地址，原因详见收藏夹
...
u8 SPI_NRF_WriteReg(uint8_t reg, uint8_t dat)
{
	...
	NRF_CE_LOW(); //必须先拉低失能NRF才能进行寄存器读写工做
	/*置低CSN，使能SPI传输*/
    NRF_CSN_LOW();
...
u8 SPI_NRF_ReadBuf(uint8_t reg, uint8_t *pBuf, uint8_t bytes)
{
	...
	status = SPI_NRF_RW(reg); 
	 ...
	 for(byte_cnt=0; byte_cnt<bytes; byte_cnt++)		  
	   pBuf[byte_cnt] = SPI_NRF_RW(NOP); //从NRF24L01读取数据，NOP = 1111 1111,为数据手册上特定的读时由SPI传的指令 
...
/*CE拉高，进入发送模式*/	
  NRF_CE_HIGH();
   ...
   delay_us(20);//还是要这样Delay才行，上面那个时间可能不够
}
...
uint8_t NRF_Tx_Dat(uint8_t *txbuf)
{
	uint8_t state;  
	 ...
	 /*ce为低，进入待机模式1*/
	NRF_CE_LOW(); //发送状态CE拉低再拉高（0 -> 1）数据手册的写法
	...
	state = SPI_NRF_ReadReg(STATUS);
	 ...
	 /*清除TX_DS或MAX_RT中断标志*/                  
	SPI_NRF_WriteReg(NRF_WRITE_REG + STATUS, state); //这里又把原来的写进去，确实也是清除中断标志位，这得结合手册来看
...
uint8_t NRF_Rx_Dat(uint8_t *rxbuf)
{
	...
	NRF_CE_HIGH();	 //进入接收状态
	 /*等待接收中断*/
	while(NRF_Read_IRQ() == 0) //接收完毕
  {
    NRF_CE_LOW();  	 //进入待机状态（只有接收前接收时CE拉高）
    /*读取status寄存器的值  */               
    state = SPI_NRF_ReadReg(STATUS);
